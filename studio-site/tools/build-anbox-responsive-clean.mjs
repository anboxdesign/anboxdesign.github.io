...
const heroRetailAssetBase = 'https://raw.githubusercontent.com/anboxdesign/anboxdesign.github.io/main/studio-site/assets/hero-retail-logos';
const heroRetailLogos = [
  { name: 'Яндекс Лавка', src: `${heroRetailAssetBase}/yandex-lavka.svg` },
  { name: 'Globus', src: `${heroRetailAssetBase}/globus.svg` },
  { name: 'Перекрёсток', src: `${heroRetailAssetBase}/perekrestok.png` },
  { name: 'Metro Cash & Carry', src: `${heroRetailAssetBase}/metro.svg` },
  { name: 'Золотое яблоко', src: `${heroRetailAssetBase}/golden-apple.png` },
  { name: 'Азбука вкуса', src: `${heroRetailAssetBase}/azbuka-vkusa.svg` },
  { name: 'Ozon', src: `${heroRetailAssetBase}/ozon.svg` },
  { name: 'Wildberries', src: `${heroRetailAssetBase}/wildberries.svg` },
];
...
function renderMobilePortfolio(markup) {
  ...
  portfolioCases.forEach((project, index) => {
    ...
    if ([4, 9, 14].includes(index)) {
      ...
      gate = `<div class="portfolio-more-slot" data-portfolio-more-slot data-portfolio-gate="${reveal}"${isHidden}><button class="portfolio-more" id="abm-portfolio-more-button-${String(reveal).padStart(2, '0')}" type="button" data-portfolio-more data-portfolio-reveal="${reveal}" aria-controls="${controls}">Смотреть ещё кейсы</button></div>`;
      gateSpacer = `<div class="portfolio-gate-spacer" data-portfolio-gate-spacer="${reveal}"${isHidden} aria-hidden="true"></div>`;
    }
    track.push(`<article class="case-slide" style="--abm-case-order:${index + 1}"${hidden} data-anbox-case="${project.number}"${hero}><figure class="case-slide__image">${renderCaseImage(project.mobile, '', alt).replace('class="" ', '')}</figure><footer class="case-slide__caption"><div class="case-slide__copy"><h3>${escapeHtml(project.title)}</h3><p class="case-slide__works">${escapeHtml(project.works || project.result)}</p></div></footer>${gate}</article>`);
    if (gateSpacer) track.push(gateSpacer);
  });
...
function heroRetailLogoImages(className, duplicate = false) {
  return heroRetailLogos.map((logo) => `<img class="${className}" src="${logo.src}" alt="${duplicate ? '' : `Логотип ${escapeHtml(logo.name)}`}" width="212" height="82"${duplicate ? '' : ' decoding="async"'}>`).join('');
}
...
.anbox-desktop-part--00.abh{transition:none!important}
  ...
  if (part === '01') {
    return `.anbox-desktop-part--01 .abh-hero__shelf-track{width:max-content!important;display:flex!important;animation:abh-retail-shelf 32s linear infinite!important}.anbox-desktop-part--01 .abh-hero__shelf-group{width:auto!important;padding:0 24px!important;display:flex!important;grid-template-columns:none!important;flex:0 0 auto!important;gap:48px!important}.anbox-desktop-part--01 .abh-hero__brand{padding:0!important}.anbox-desktop-part--01 .abh-hero__brand img{width:auto!important;max-width:180px!important;height:32px!important}.anbox-desktop-part--01 .abh-hero__brand:first-child img{transform:translateY(1px)}.anbox-desktop-part--01 .abh-hero__brand:nth-child(2) img{height:46px!important}.anbox-desktop-part--01 .abh-hero__brand:nth-child(3) img{filter:brightness(1.06);mix-blend-mode:multiply}.anbox-desktop-part--01 .abh-hero__brand:nth-child(7) img{height:25px!important}.anbox-desktop-part--01 .abh-hero__brand:nth-child(8) img{height:23px!important}@keyframes abh-retail-shelf{from{transform:translate3d(0,0,0)}to{transform:translate3d(-33.333333%,0,0)}}`;
  }
  if (part === '02') {
...
function composedMobileCss(part) {
  if (part === '03') return `.anbox-mobile-part--03{--abm-gallery-top:4.25rem}.anbox-mobile-part--03,.anbox-mobile-part--03 .portfolio,.anbox-mobile-part--03 .case-viewport,.anbox-mobile-part--03 .case-track{max-width:100%!important;overflow-x:clip!important}.anbox-mobile-part--03 .case-track{width:100%!important;display:block!important;transform:none!important;transition:none!important;will-change:auto!important}.anbox-mobile-part--03 .case-viewport,.anbox-mobile-part--03 .case-slide,.anbox-mobile-part--03 .case-slide__image,.anbox-mobile-part--03 .case-slide__caption{border-radius:0!important}.anbox-mobile-part--03 .case-slide,.js .anbox-mobile-part--03 .case-slide{position:sticky!important;top:var(--abm-gallery-top)!important;right:auto!important;bottom:auto!important;left:auto!important;z-index:var(--abm-case-order)!important;width:100%!important;max-width:100%!important;min-height:calc(100vh - var(--abm-gallery-top))!important;min-height:calc(100svh - var(--abm-gallery-top))!important;margin-inline:0!important;display:flex!important;flex-direction:column!important;overflow:clip!important;scroll-margin-top:var(--abm-gallery-top);isolation:isolate}.anbox-mobile-part--03 .case-slide[hidden]{display:none!important}.anbox-mobile-part--03 .case-slide.is-past{visibility:hidden!important;pointer-events:none!important;isolation:auto!important}.anbox-mobile-part--03 .case-slide+.case-slide{box-shadow:0 -14px 30px rgba(0,0,0,.3)}.anbox-mobile-part--03 .case-slide__image,.anbox-mobile-part--03 .case-slide__image img,.anbox-mobile-part--03 .case-slide__caption{box-sizing:border-box!important;width:100%!important;max-width:100%!important;margin-inline:0!important;inset-inline:auto!important}.anbox-mobile-part--03 .case-slide__image{flex:1 1 0!important;min-height:clamp(360px,56svh,560px)!important}.anbox-mobile-part--03 .case-slide__image img{display:block!important}.anbox-mobile-part--03 .case-slide__caption,.anbox-mobile-part--03 .portfolio-caption-dock{grid-template-columns:minmax(0,1fr)!important}.anbox-mobile-part--03 .case-slide__caption h3,.anbox-mobile-part--03 .portfolio-caption-dock strong{display:flex!important;align-items:center!important;gap:10px!important}.anbox-mobile-part--03 .case-slide__caption h3::after,.anbox-mobile-part--03 .portfolio-caption-dock strong::after{width:42px;height:2px;flex:0 0 42px;background:var(--purple,var(--abx-purple,#ad95ee));content:""}.anbox-mobile-part--03 .portfolio-more-slot{position:absolute!important;z-index:3!important;inset:0!important;min-height:0!important;padding:0 max(var(--pad),env(safe-area-inset-right)) 0 max(var(--pad),env(safe-area-inset-left))!important;display:grid!important;place-items:center!important;background:transparent!important;pointer-events:none!important;overflow-anchor:none}.anbox-mobile-part--03 .portfolio-more-slot[hidden]{display:none!important}.anbox-mobile-part--03 .portfolio-more{position:relative!important;top:auto!important;z-index:1!important;width:100%!important;border-color:var(--purple,var(--abx-purple,#ad95ee))!important}.anbox-mobile-part--03 .portfolio.is-caption-docked .case-slide__caption{display:grid!important;visibility:hidden!important;pointer-events:none!important}@media(prefers-reduced-motion:reduce){.anbox-mobile-part--03 .case-slide+.case-slide{box-shadow:none}.anbox-mobile-part--03 .portfolio-more{transition:none!important}}`;
  ...
  if (part === '10') return `.anbox-mobile-part--10{--white:#fff;--ink:var(--abx-ink,#151716);--line:rgba(21,23,22,.14);--muted:#6f6c70;--pad:clamp(16px,4.6vw,24px);--abx-mob-motion-fast:180ms cubic-bezier(.22,1,.36,1);min-width:0;margin:0;background:var(--white);color:var(--ink);font-family:"Onest",Arial,sans-serif;font-size:16px;line-height:1.5;overflow-wrap:anywhere;-webkit-text-size-adjust:100%;text-size-adjust:100%}.anbox-mobile-part--10,.anbox-mobile-part--10 *,.anbox-mobile-part--10::before,.anbox-mobile-part--10 *::before,.anbox-mobile-part--10::after,.anbox-mobile-part--10 *::after{box-sizing:border-box}.anbox-mobile-part--10 a{color:inherit;-webkit-tap-highlight-color:transparent}.anbox-mobile-part--10 svg{display:block;max-inline-size:100%}.anbox-mobile-part--10 .site-footer{width:100%;margin:0;padding:28px var(--pad) max(24px,env(safe-area-inset-bottom));background:var(--white)!important}.anbox-mobile-part--10 .footer-inner{width:min(100%,48rem);margin-inline:auto}.anbox-mobile-part--10 .footer-top{display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:12px}.anbox-mobile-part--10 .brand{min-height:44px;display:inline-flex;flex:0 0 auto;align-items:center;color:currentColor;text-decoration:none}.anbox-mobile-part--10 .footer-brand__logo{width:clamp(148px,41vw,168px);max-width:100%;height:auto;display:block;overflow:visible}.anbox-mobile-part--10 .footer-tagline{max-width:26ch;margin:14px 0 0;color:var(--muted);font-size:15px;line-height:1.45;text-wrap:pretty}.anbox-mobile-part--10 .footer-meta{margin-top:22px;padding-top:16px;display:grid;gap:8px;border-top:1px solid var(--line);color:var(--muted);font-size:12px;line-height:1.4}.anbox-mobile-part--10 .footer-meta__identity{min-width:0;display:grid;gap:5px}.anbox-mobile-part--10 .footer-meta a{min-height:44px;display:inline-flex;align-items:center;width:fit-content;max-width:100%;text-decoration:none}.anbox-mobile-part--10 .socials{margin:0;padding:0;display:flex;gap:8px}.anbox-mobile-part--10 .socials a{width:44px;height:44px;min-width:44px;min-height:44px;display:grid;place-items:center;border:1px solid rgba(21,23,22,.2);border-radius:50%;background:transparent;color:var(--ink);text-decoration:none;transition:color var(--abx-mob-motion-fast),background-color var(--abx-mob-motion-fast),border-color var(--abx-mob-motion-fast),transform var(--abx-mob-motion-fast)}.anbox-mobile-part--10 .socials svg{width:20px;height:20px;display:block}.anbox-mobile-part--10 .social-icon--fill{fill:currentColor}.anbox-mobile-part--10 .social-icon--stroke{fill:none;stroke:currentColor}.anbox-mobile-part--10 :focus-visible{outline:3px solid var(--abx-purple-strong,#7658d0);outline-offset:3px}.anbox-mobile-part--10 .socials a:active{border-color:var(--ink);background:var(--ink);color:var(--white);transform:scale(.94)}@media(max-width:20rem){.anbox-mobile-part--10 .footer-top{align-items:flex-start}.anbox-mobile-part--10 .socials{gap:6px}}@media(min-width:36rem){.anbox-mobile-part--10 .footer-meta{grid-template-columns:minmax(0,1fr) auto;align-items:end}}@media(prefers-reduced-motion:reduce){.anbox-mobile-part--10 .socials a{transition:none}}`;
  return '';
}
...
function finalMobileCss(part) {
  ...
  if (part === '07') return `${css}.anbox-mobile-part--07 .team-grid{grid-template-columns:minmax(0,1fr)!important;gap:12px!important}.anbox-mobile-part--07 .person-card,.anbox-mobile-part--07 .person-card:first-child,.anbox-mobile-part--07 .team-grid .person-card:last-child{grid-column:auto!important}.anbox-mobile-part--07 .person-photo{height:clamp(200px,62vw,320px)!important;aspect-ratio:auto!important}.anbox-mobile-part--07 .person-card:first-child .person-photo{height:clamp(220px,66vw,340px)!important;aspect-ratio:auto!important}.anbox-mobile-part--07 .person-card:nth-child(2) .person-photo img{object-position:center 24%!important}.anbox-mobile-part--07 .person-card:nth-child(3) .person-photo img{object-position:center 22%!important}.anbox-mobile-part--07 .person-card__caption{padding:14px 16px 16px!important}.anbox-mobile-part--07 .person-card:first-child .person-card__caption{padding:16px 18px 18px!important}.anbox-mobile-part--07 .person-card h3{font-size:18px!important}.anbox-mobile-part--07 .person-card__caption>.person-card__role{margin-top:5px!important;font-size:clamp(13px,3.8vw,14px)!important;line-height:1.42!important}.anbox-mobile-part--07 .person-card__caption>:is(.person-card__proof,.person-card__meta){font-size:clamp(12px,3.55vw,13px)!important;line-height:1.42!important}.anbox-mobile-part--07 .person-card__caption>.person-card__meta{margin-top:5px!important}`;
...
function mobileRuntimeFor(part) {
  ...
  if (part === '00') {
    ...
    code = `${sharedReducedMotion}${sliceRuntime("const portfolio = document.querySelector('#work');", "const serviceTabs = [...document.querySelectorAll('.service-tab')];")}`
      ...
      .replace(/      const servicesSection = document\.querySelector\('#services'\);\r?\n/, '')
      ...
      .replace(/      const portfolioDockLink = portfolio\.querySelector\('\[data-portfolio-dock-link\]'\);\r?\n/, '')
      .replace(/          const link = activeSlide\.querySelector\('\.case-slide__link'\);\r?\n/, '')
      .replace(/          portfolioDockLink\.href = link\.href;\r?\n          portfolioDockLink\.target = link\.target;\r?\n/, '');
  } else if (part === '04') {
...
html.anbox-gallery-editor .anbox-mobile-part--03 :is(.portfolio-more-slot,.portfolio-gate-spacer,.portfolio-caption-dock){display:none!important}
...
[data-anbox-reveal-ready="1"]:is([data-anbox-reveal-kind="display"],[data-anbox-reveal-kind="heading"])[data-anbox-reveal-settled="1"]{-webkit-mask:none!important;mask:none!important}
...
function settleHeading(state,animation){if(animation&&state.headingAnimation!==animation)return;var active=state.headingAnimation;state.headingAnimation=null;if(active){try{active.cancel()}catch(error){}}state.node.classList.add('anbox-reveal-in');state.node.dataset.anboxRevealSettled='1'}
function startHeading(state,delay){delete state.node.dataset.anboxRevealSettled;measureHeading(state);if(state.headingAnimation)state.headingAnimation.cancel();var total=w.innerWidth<=640?860:900;var frames=headingFrames(state,total);if(reduced||!state.node.animate||!frames.length)return;var animation=state.node.animate(frames,{duration:total,delay:delay,easing:'linear',fill:'both'});state.headingAnimation=animation;animation.finished.then(function(){settleHeading(state,animation)}).catch(function(){})}
function setState(state,on,edge){var node=state.node;var delay=on?(edge==='top'?state.delayTop:state.delayBottom):0;var isHeading=state.kind==='display'||state.kind==='heading';node.dataset.anboxRevealEdge=edge;node.style.setProperty('--abx-reveal-delay',delay+'ms');node.dataset.anboxRevealMoving='1';w.clearTimeout(state.motionTimer);if(state.headingAnimation){state.headingAnimation.cancel();state.headingAnimation=null}if(on){if(isHeading){node.classList.add('anbox-reveal-in');startHeading(state,delay)}else{node.style.setProperty('transition','none','important');node.classList.remove('anbox-reveal-in');void node.offsetWidth;node.style.removeProperty('transition');void node.offsetWidth;node.classList.add('anbox-reveal-in')}}else{if(isHeading)delete node.dataset.anboxRevealSettled;node.classList.remove('anbox-reveal-in')}var budget=isHeading?(w.innerWidth<=640?860:900)+delay+48:motionBudget(node);state.motionTimer=w.setTimeout(function(){if(isHeading&&on&&state.on)settleHeading(state);delete node.dataset.anboxRevealMoving},budget)}
function settleVisibleHeadings(){states.forEach(function(state){if((state.kind==='display'||state.kind==='heading')&&state.on){w.clearTimeout(state.motionTimer);settleHeading(state);delete state.node.dataset.anboxRevealMoving}})}
...
document.addEventListener('visibilitychange',function(){if(!document.hidden)settleVisibleHeadings()});w.addEventListener('pageshow',function(event){if(event.persisted)settleVisibleHeadings()});
